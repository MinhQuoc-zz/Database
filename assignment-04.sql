...
SELECT category_name, COUNT(question_id) AS total_questions
FROM category_question 
LEFT JOIN question  USING (category_id)
GROUP BY category_name
ORDER BY total_questions DESC;
-- Question 7: Thông kê mỗi Question(câu hỏi) được sử dụng trong bao nhiêu Exam
-- Thống kê theo cái gì thì nhóm theo cái đấy
SELECT question_id,COUNT(exam_id)
FROM exam_question
RIGHT JOIN question USING (question_id)
GROUP BY question_id;

SELECT question_id, content, COUNT(exam_id) AS total_exams
FROM question 
...
SELECT question_id, content, COUNT(answer_id) AS total_answers
FROM question 
...
-- Thống kê mỗi loại câu hỏi có bao nhiêu câu hỏi
