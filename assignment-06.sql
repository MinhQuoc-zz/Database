...
-- account thuộc phòng ban đó.
	DROP PROCEDURE IF EXISTS qs_01;
    DELIMITER $$
    CREATE PROCEDURE qs_01(IN in_department_name VARCHAR(50))
    BEGIN
...
DROP PROCEDURE IF EXISTS qs_02;
DELIMITER $$
CREATE PROCEDURE qs_02()
BEGIN

	SELECT 
        `group`.*,COUNT(account_id) AS total_accounts
    FROM `group` 
...
--  DECLARE v_current_month INT DEFAULT MONTH(CURRENT_DATE) ;
    ...
    SELECT  MONTH(CURRENT_DATE) INTO v_current_month;
	WITH c1 AS(
    SELECT *
    FROM question
...
DELIMITER $$
CREATE PROCEDURE qs_04(OUT out_type_id INT)
BEGIN
	WITH c1 AS (
...
DELIMITER $$
CREATE PROCEDURE qs_06(IN search_string VARCHAR(50))
BEGIN
...
-- username sẽ giống email nhưng bỏ phần @..mail đi substringIndex()
