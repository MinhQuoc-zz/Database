DROP DATABASE IF EXISTS thuchanh_01;
...
WHERE ga.joined_date < '2019-12-20';
...
-- Tính tổng số câu hỏi trong từng loại câu hỏi (type_question).
SELECT type_id , COUNT(question_id)
FROM question
GROUP BY type_id;
-- Đếm số lượng group được tạo bởi mỗi tài khoản.
SELECT creator_id, COUNT(*) AS total_groups
FROM `group`
GROUP BY creator_id;     
-- 2 Lấy số lượng nhân viên tham gia từng group.

SELECT group_id, COUNT(account_id) AS total_members
FROM group_account
GROUP BY group_id;
-- Đếm số lượng câu trả lời đúng và sai cho từng câu hỏi.
SELECT question_id, is_correct, COUNT(*) AS total_answers
FROM answer
GROUP BY question_id, is_correct;

-- Tính tổng thời gian thi của từng mã đề thi (exam).

SELECT exam_id ,SUM(duration) AS sum_duration
FROM exam
GROUP BY exam_id;

-- Lấy thông tin phòng ban có nhiều nhân viên nhất.
...
-- Tìm các phòng ban có nhiều hơn 3 nhân viên.
SELECT position_id, COUNT(account_id) account_count
FROM account
GROUP BY position_id
HAVING account_count >= 3;

-- Lấy ra các đề thi có số lượng câu hỏi lớn nhất.
...
-- Lấy ra các tài khoản đã tham gia ít nhất 2 group.
SELECT account_id, COUNT(account_id) AS account_count
FROM group_account
GROUP BY account_id
HAVING account_count >= 2;

-- Câu hỏi cực khó (Chuyên sâu):
-- Lấy ra các group có tổng số thành viên lớn hơn số lượng thành viên trung bình của tất cả các group.

SELECT group_id, COUNT(account_id) AS group_count
FROM group_account
GROUP BY group_id
HAVING group_count > (SELECT AVG(member_count) FROM (SELECT COUNT(account_id) AS member_count
					FROM group_account
					GROUP BY group_id) AS subquery);

-- Đếm số lượng đề thi cho từng loại câu hỏi (type_id), chỉ hiển thị những loại có nhiều hơn 5 đề thi.
SELECT type_id, COUNT(question_id) AS question_count
FROM exam 
JOIN exam_question USING(exam_id)
JOIN question  USING (question_id)
GROUP BY type_id
HAVING question_count > 5;

-- Lấy ra tất cả các phòng ban có tổng số nhân viên bằng với phòng ban có ít nhân viên nhất.
SELECT department_id, COUNT(*) AS total_employees
FROM account
GROUP BY department_id
HAVING total_employees = (SELECT COUNT(account_id) AS account_count
						FROM group_account
						GROUP BY group_id
